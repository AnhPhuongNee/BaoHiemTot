...
const Header = () => {
  const [isPersonalDropdownVisible, setPersonalDropdownVisible] = useState(false);
  const [isBusinessDropdownVisible, setBusinessDropdownVisible] = useState(false);
  const [isMobileMenuVisible, setMobileMenuVisible] = useState(false); // Thêm trạng thái cho menu di động
  ...
  const toggleMobileMenu = () => {
    setMobileMenuVisible(!isMobileMenuVisible); // Chuyển đổi trạng thái mở/đóng menu di động
  };
  ...
  return (
    <header className="header">
      ...
      {/* Nút hamburger sẽ hiển thị trên màn hình nhỏ */}
      ...
      <nav className={isMobileMenuVisible ? "nav-links-mobile" : "nav-links"}>
        <ul className="nav-links">
          <li>
            <div className="dropdown" onMouseEnter={handlePersonalDropdown} onMouseLeave={handlePersonalDropdown}>
              <button className="dropbtn">Bảo hiểm cá nhân</button>
